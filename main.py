...
class MarkerData(db.Model):
    ...
    category = db.Column(db.String(100), nullable=False)  # IoT data, crop data, etc.
    data_value = db.Column(db.Text, nullable=False)  # The actual data
    notes = db.Column(db.Text, nullable=True)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
...
def add_marker():
    ...
    new_marker = Marker(
        ...
        address=data['address']   # Save category data
    )
...
def edit_marker(marker_id):
    ...
    marker.address = data.get('address', marker.address)
    db.session.commit()
    ...
    incidents_data = [
        {
            ...
            "report_text": incident.report_text,
            "id": incident.id,
            "timestamp": incident.timestamp.strftime('%Y-%m-%d %H:%M:%S') if incident.timestamp else None
        }
        for incident in incidents
    ]
    return jsonify(incidents_data)
...
@app.route('/get_marker_data/<int:marker_id>')
def get_marker_data(marker_id):
    data = MarkerData.query.filter_by(marker_id=marker_id).all()
    return jsonify([{
        'id': item.id,
    ...
    try:
        ...
        if 'image' in request.files:
            ...
            if image.filename:
                ...
                unique_filename = f"{marker_id}_{int(time.time())}_{secrets.token_hex(8)}_{secure_filename(image.filename)}"
                
                # Ensure the directory exists
                os.makedirs(os.path.join(app.config['UPLOAD_FOLDER'], 'markerdata'), exist_ok=True)
                
                # Full path for saving
                full_save_path = os.path.join(app.config['UPLOAD_FOLDER'], 'markerdata', unique_filename)
                
                # Save the image
                image.save(full_save_path)
                
                # Set image path for database
                image_path = f"/static/uploads/markerdata/{unique_filename}"
...
@app.route('/delete_marker_data/<int:data_id>', methods=['DELETE'])
def delete_marker_data(data_id):
    try:
        data = MarkerData.query.get(data_id)
        if data.image_path:
            # Delete the image file
            file_path = os.path.join(app.config['UPLOAD_FOLDER'], data.image_path.split('/')[-1])
...
def get_alerts():
    url = "https://asdma.komunidad.co:8443/enterprise/api/getAlertsList"
    headers = {
        "Authorization": "Bearer CFGlFLlJQcP6RcHgz1RmfQFbxrYRFHCX"
    }
...
def report():
    try:
        ...
        # Return JSON response with successful submission details
        return jsonify({
            "success": True, 
            ...
            "incident": {
                ...
                "media_path": new_incident.media_path
            }
        }), 201
...
def dashboard():
    ...
    # Get responses count
        response_count = db.session.query(QResponses).count()
...
@app.route('/incident-analysis', methods=['GET', 'POST'])
def incident_analysis():
    return render_template('incident_analysis.html')
...
def analyze_incidents():
    ...
    if date_range:
        ...
        query = query.filter(Incident.timestamp.between(start_date, end_date))
    ...
    analysis_prompts = {
        'trend_analysis': """Analyze the following incident data to identify key trends:
        Provide insights into:
        - Frequency of incidents over time
        - Most common incident categories
        - Notable patterns or changes in incident rates
        """,
        'location_patterns': """Analyze the incident data to uncover location-based patterns:
        Focus on:
        - Areas with highest incident concentrations
        - Correlation between locations and incident types
        - Geographical risk assessment
        """,
        'category_distribution': """Examine the distribution of incident categories:
        Provide a comprehensive breakdown of:
        - Percentage distribution across different incident types
        - Most frequent and least frequent categories
        - Any surprising or unexpected category distributions
        """,
        'risk_assessment': """Conduct a detailed risk assessment based on the incident data:
        Analyze:
        - Potential high-risk areas or scenarios
        - Severity and frequency of different incident types
        - Recommendations for risk mitigation
        """,
        'temporal_analysis': """Perform a temporal analysis of the incidents:
        Investigate:
        - Time of day with most incidents
        - Day of week with highest incident rates
        - Seasonal trends or patterns
        """,
        'predictive_insights': """Generate predictive insights from the incident data:
        Provide:
    ...
    # Prepare full prompt
    standard_prompt = f"""Incident Analysis Prompt:
Analysis Type: {analysis_type}
Specific Instructions: {analysis_prompts.get(analysis_type, 'Provide a general analysis of the incidents')}

Incident Data:
{json.dumps(incident_data, indent=2)}"""

    # Combine standard prompt with custom prompt if provided
    if custom_prompt:
        full_prompt = f"""{standard_prompt}

Additional Custom Guidance from User:
{custom_prompt}

Please incorporate the custom guidance into your analysis while maintaining the core analysis objectives."""
    else:
...
def parse_date_range(date_range):
    ...
    if date_range == 'last_week':
        ...
        _, start_str, end_str = date_range.split(':')
...
Create a comprehensive, unified analysis that captures the key insights from all chunks."""
    ...
    def sentiment_analysis(self, custom_prompt: str = '') -> Dict[str, Any]:
        ...
        """
        # Combine first 10 responses to give context
        sample_responses = [r.response_text for r in self.responses[:10]]
        
        prompt = f"""Conduct a comprehensive sentiment analysis of the following survey responses:

Responses: {json.dumps(sample_responses)}

Provide a detailed analysis that covers:
- The overall emotional tone of the responses
- Breakdown of sentiment types (positive, negative, neutral)
- Key emotional themes and underlying sentiments
...
Use bullet points and be concise, offering deep, contextual understanding of the emotional content."""
    ...
    def theme_extraction(self) -> Dict[str, Any]:
        """
        Extract key themes using OpenAI
        """
        # Combine responses
        all_responses = [r.response_text for r in self.responses]
        
        prompt = f"""Perform an in-depth thematic analysis of the following survey responses:

Responses: {json.dumps(all_responses)}

Your analysis should:
- Identify the most significant and recurring themes
- Provide nuanced descriptions of each identified theme
- Explore the interconnections between these themes
...
Use bullet points and be concise. Use data and mention data that reveals the deeper narrative within the responses."""
    ...
    def demographic_insights(self, custom_prompt: str = '') -> Dict[str, Any]:
        """
        Generate demographic insights using OpenAI
        """
        # Collect demographic information
        locations = [r.location for r in self.responses if r.location]
        languages = [r.language for r in self.responses if r.language]
        
        prompt = f"""Conduct a comprehensive demographic analysis of the survey responses:

Locations: {json.dumps(locations)}
Languages: {json.dumps(languages)}

Develop a sophisticated analysis that:
- Provides an in-depth examination of the geographic distribution
- Explores the linguistic diversity of the respondents
- Uncovers subtle demographic patterns and correlations
...
Use concise data and bullet points."""
    ...
    def geographic_distribution(self, custom_prompt: str = '') -> Dict[str, Any]:
        """
        Analyze geographic distribution using OpenAI
        """
        ...
        prompt = f"""Perform a comprehensive geographic distribution analysis of the survey responses:

Geographic Data: {json.dumps(geo_data)}

Your analysis should:
- Provide a detailed breakdown of geographic spread
- Identify concentration points and spatial patterns
- Explore potential regional variations in responses
...
Be concise and use bullet points and summaries and numbers."""
    ...
    def language_analysis(self, custom_prompt: str = '') -> Dict[str, Any]:
        """
        Perform language analysis using OpenAI
        """
        ...
        prompt = f"""Conduct an advanced linguistic analysis of the survey responses:

Language Responses: {json.dumps(language_responses)}

Your comprehensive analysis should:
- Examine the linguistic diversity of the responses
- Analyze communication complexity and style
- Identify unique linguistic patterns and nuances
...
Be concise, brief and use summaries and bullet points and numbers."""
    ...
    def correlation_analysis(self, custom_prompt: str = '') -> Dict[str, Any]:
        """
        Generate correlation insights using OpenAI
        """
        ...
        prompt = f"""Perform an advanced correlation analysis of the survey responses:

Correlation Data: {json.dumps(correlation_data)}

Your analysis should:
- Uncover intricate correlations between different response attributes
- Identify subtle and significant patterns in the data
- Explore unexpected relationships and insights
- Provide a nuanced interpretation of the interconnected response characteristics
- Offer strategic recommendations based on the observed correlations
...
def get_surveys():
    try:
        ...
        # Debug print
        for survey in surveys:
            print(f"Survey - ID: {survey.id}, Title: {survey.title}, Created At: {survey.created_at}")
        ...
        return jsonify([
            {
                ...
                'status': survey.status or 'Unknown'
            } 
            for survey in surveys
        ])
    except Exception as e:
...
def analyze_survey():
    ...
    try:
        ...
        analysis_methods = {
            ...
            'theme_extraction': analyzer.theme_extraction,
